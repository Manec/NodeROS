...
"""autogenerated by genpy from global_planner/PathWithRiskMsg.msg. Do not edit."""
import sys
...
float64 w
"""
  __slots__ = ['header','poses','risks']
  _slot_types = ['std_msgs/Header','geometry_msgs/PoseStamped[]','float64[]']
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      length = len(self.risks)
      buff.write(_struct_I.pack(length))
      pattern = '<%sd'%length
      buff.write(struct.pack(pattern, *self.risks))
  ...
  def serialize_numpy(self, buff, numpy):
    ...
    try:
      ...
      buff.write(_struct_I.pack(length))
      pattern = '<%sd'%length
      buff.write(self.risks.tostring())
    except struct.error as se: self._check_types(struct.error("%s: '%s' when writing '%s'" % (type(se), str(se), str(locals().get('_x', self)))))
    except TypeError as te: self._check_types(ValueError("%s: '%s' when writing '%s'" % (type(te), str(te), str(locals().get('_x', self)))))
